...
Ghostty, Herdr, Zsh, Sheldon, Starship, tmux, Vim, Neovim, and Codex.
...
Codex reads shared defaults from `~/.codex/config.toml` and applies a profile
such as `~/.codex/review.config.toml` when started with `--profile review`.
If `config.toml` already exists, merge the portable root keys and tables instead
...
configuration uses Ghostty's built-in JetBrains Mono and Nerd Font Symbols,
with macOS system fonts providing Japanese and emoji fallback. It also uses the
Catppuccin Frappe theme and defines clipboard, cursor, window, shell integration,
and split navigation behavior. Full-screen application backgrounds extend into
the balanced window padding, and unfocused splits fade toward the Frappe Mantle
color.
